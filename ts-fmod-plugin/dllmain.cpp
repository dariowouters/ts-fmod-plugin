...
SCSAPI_VOID telemetry_tick(const scs_event_t event, const void* const event_info, scs_context_t context)
{
    ...
    fmod_manager_instance->set_event_parameter("engine/engine", "brake", telemetry_data.engine_brake ? 1.0f : 0.0f);
    ...
    if (reinterpret_cast<economy_base_t*>(economy_base_offset) != nullptr && economy_base_offset != NULL)
    {
        ...
        auto* const unk = economy_base->get_unk();
        if (unk != nullptr)
        {
            auto* const some_arr = unk->get_some_arr_ptr();
            if (some_arr != nullptr)
            {
                auto* const some_truck_telem_parent = some_arr->get_some_truck_telem_data_parent();
                if (some_truck_telem_parent != nullptr)
                {
                    auto* const truck_telem_data = some_truck_telem_parent->get_some_truck_telem_data();
        ...
        auto* const unk_window_parent = economy_base->get_unk_window_parent();
...
SCSAPI_RESULT scs_telemetry_init(const scs_u32_t version, const scs_telemetry_init_params_t* const params)
{
    ...
    const auto addr = pattern::scan("48 8B 05 ? ? ? ? 48 8B D9 8B 90 ? ? ? ? 48 8B 80 ? ? ? ? 48 8B 88 ? ? ? ? E8", game_base, image_size);
